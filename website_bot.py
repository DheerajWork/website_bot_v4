...
"""
website_bot.py
Fully Updated Version — with guaranteed social media extraction:
✓ Extract social icons (svg/img)
✓ Extract meta tags (og:url, sameAs)
✓ Extract anchor links with social domains
"""
...
try:
    ...
    raise SystemExit("Install: pip install playwright beautifulsoup4 chromadb openai")
...
def extract_social_links_from_html(html):
    ...
    # ---- 3) JSON-LD Social Profiles (most accurate) ----
...
def select_main_pages(urls, base):
    ...
    for u in urls:
        u2 = u.lower()
        if ("about" in u2 and u not in pages):
            pages.append(u)
        if ("contact" in u2 and u not in pages):
            pages.append(u)
...
if __name__ == "__main__":
    ...
    # merge AI + real scraped social media (strong guarantee)
